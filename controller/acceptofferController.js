...
class acceptofferController {
  ...
  static getofferbyinflu = async (req, res) => {
    try {
      ...
      const offerbyinflu = await acceptoffer
        .find({
          userId,
      ...
      const validOffers = offerbyinflu.filter(
      ...
      for (const offer of validOffers) {
        ...
        if (!seenOfferIds.has(offerIdStr)) {
          ...
          const businessDetails = await user
            .findById(offer.offerId.businessId)
            .select("-userPassword")
            .lean();
  ...
  static getofferbybusiness = async (req, res) => {
    try {
      ...
      const userlist = await acceptoffer
        .find({ businessId })
        .populate("offerId")
        .populate("userId");
